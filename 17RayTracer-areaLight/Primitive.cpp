...
bool Instance::shadowHit(Ray &ray){
	...
	if (m_primitive->bounds && !m_primitive->box.intersect(transformedRay)){
		...
		return m_primitive->shadowHit(transformedRay);
	}
}
...
bool CompoundedObject::shadowHit(Ray &_ray){

	bool hitObject = false;
	
	for (unsigned int i = 0; i < m_primitives.size(); i++){

		hitObject = hitObject || m_primitives[i]->shadowHit(_ray);
	}
...
bool Triangle::shadowHit(Ray &ray){
...
bool Annulus::shadowHit(Ray &ray){
...
bool Rectangle::shadowHit(Ray &ray){
...
bool OpenCylinder::shadowHit(Ray &ray){
...
bool OpenCone::shadowHit(Ray &ray){
...
bool SolidCylinder::shadowHit(Ray &ray){

	return CompoundedObject::shadowHit(ray);
}
...
bool SolidCone::shadowHit(Ray &ray){
...
bool BeveledCylinder::shadowHit(Ray &ray){
