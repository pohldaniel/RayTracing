...
NormalMap::NormalMap(const char* path){
	...
	m_normalMap = std::shared_ptr<ImageTexture>(new ImageTexture(path));
}

NormalMap::~NormalMap(){}

void NormalMap::setNormalMap(std::shared_ptr<ImageTexture> normalMap){
...
Color NormalMap::shade(Hit &hit){
	...
	// -w_0
	Vector3f incidentRay = (hit.transformedRay.direction).normalize();
	//Color tmp = m_normalMap->getSmoothTexel(hit.u, hit.v);
	Color tmp = m_normalMap->getTexel(hit.u, hit.v, hit.hitPoint);
...
Color Reflective::shade(Hit &hit){
	...
	// to get the reflected ray in eyespace use the transformed noraml
	// w_i
